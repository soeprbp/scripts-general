...
if (-not $dbEntry) {
    Write-Host "ERROR: '$targetDB' not found in DPA. Available instances:" -ForegroundColor Red
...
# ---- Step 3: Pull Hourly Wait Data ----
Write-Host "Pulling hourly wait data for last $hoursBack hours..." -ForegroundColor Cyan
...
for ($h = 0; $h -lt $hoursBack; $h++) {
    ...
    $url       = $baseURL + "databases/$databaseId/wait-time/history?startTime=$startStr&endTime=$endStr&intervalUnit=HOUR"
...
$labelsList = $hourlyData | ForEach-Object { "`"$($_.Hour)`"" }
$labels     = $labelsList -join ","
$dpList     = $hourlyData | ForEach-Object { "$($_.WaitSecs)" }
$dataPoints = $dpList -join ","

# ---- Step 6: Build HTML Table Rows ----
$tableRowsList = foreach ($row in $hourlyData) {
    if ($peakWait -gt 0) {
        $bar = [Math]::Min([Math]::Round(($row.WaitSecs / $peakWait) * 100), 100)
    }
    else {
        $bar = 0
    }

    if ($row.WaitSecs -ge ($peakWait * 0.85)) {
        $rowStyle = " style='background:#fff5f5'"
    }
    elseif ($row.WaitSecs -ge ($peakWait * 0.60)) {
        $rowStyle = " style='background:#fffbf0'"
    }
    else {
        $rowStyle = ""
    }

    "<tr$rowStyle><td>$($row.DateTime)</td><td>$($row.WaitSecs)s</td><td><div class='bar-wrap'><div class='bar' style='width:$($bar)%'></div></div></td></tr>"
}
$tableRows = $tableRowsList -join "`n"
...
<div class="container">
  ...
  <div class="panel">
    <h2>&#9201; Wait Time by Hour (seconds) &#8212; Blue: Normal &nbsp;|&nbsp; Orange: Elevated &nbsp;|&nbsp; Red: Peak</h2>
  ...
  new Chart(document.getElementById('perfChart'), {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: function(c) { return ' Wait: ' + c.parsed.y + 's'; }
          }
        }
      },
      scales: {
